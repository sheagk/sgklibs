...
def viz_part_indices(hal_i, part, hal, species_name='star', kind='3panel',
    ...
    if recenter:
        ...
        print("Center moved by {} kpc".format(
            fast_dist(old_pos, center_position)))
    ...
    if rotate:
        if rot_vec is None:
            RV, EV, AR = ut.particle.get_principal_axes(
                part, species=species_name, center_position=center_position, distance_max=bsize*rotate_cut)
        else:
            RV = rot_vec
        rot_pos = ut.particle.get_distances_wrt_center(
            part, species=species_name, center_position=center_position, distance_kind='vector', rotation=RV)
        particles_in_box = boxmsk(rot_pos, [0, 0, 0], bsize)  # already shifted
    else:
        rot_pos = part[species_name]['position'] - center_position
        particles_in_box = boxmsk(
            part[species_name]['position'], center_position, bsize)
    ...
    if verbose:
        ...
        print("Have {} total particles in the box".format(
            np.count_nonzero(particles_in_box)))
    ...
    if kind == '3panel':
        ...
        # x vs y -- shares the xaxis with the plot above it
        ax3 = fig.add_subplot(223, facecolor=bgcolor,
                              sharex=ax1, aspect='equal')
        # z vs y -- shares the y axis with the plot beside it
        ax4 = fig.add_subplot(224, facecolor=bgcolor,
                              sharey=ax3, aspect='equal')
    ...
    if ided_part_method == 'hist':
        if kind == '3panel':
            ...
            hist, xe, ye = np.histogram2d(
                xp, zp, bins=nbins, weights=my_weights)
            ...
            hist, xe, ye = np.histogram2d(
                xp, yp, bins=nbins, weights=my_weights)
            hist = convert_to_per_pc2(hist, xe[1]-xe[0])
            Y_eo, X_eo = np.meshgrid(xe, ye)
            ...
            # last, z vs y in ax4
            hist, xe, ye = np.histogram2d(
                zp, yp, bins=nbins, weights=my_weights)
            hist = convert_to_per_pc2(hist, xe[1]-xe[0])
            Y_eo, X_eo = np.meshgrid(xe, ye)
            ...
            if kind == 'edgeon':
                ...
                yi = yp
            hist, xe, ye = np.histogram2d(
                xi, yi, bins=nbins, weights=my_weights)
            hist = convert_to_per_pc2(hist, xe[1]-xe[0])
            Y_eo, X_eo = np.meshgrid(xe, ye)
            ...
            if cbar:
                ...
                cbar.set_label(
                    r'projected density [$M_\odot$/pc$^2$]', labelpad=15)
        ...
        print("!! don't know method for plotting particles as {}; not plotting selected particles".format(
            mine_method))
    ...
    if nearby_part_method == 'hist':
        if kind == '3panel':
            # first, x vs z in ax1
            hist, xe, ye = np.histogram2d(
                xp, zp, bins=nbins, weights=other_weights)
            hist = convert_to_per_pc2(hist, xe[1]-xe[0])
            Y_eo, X_eo = np.meshgrid(xe, ye)
            ...
            # second, x vs y in ax3
            hist, xe, ye = np.histogram2d(
                xp, yp, bins=nbins, weights=other_weights)
            hist = convert_to_per_pc2(hist, xe[1]-xe[0])
            Y_eo, X_eo = np.meshgrid(xe, ye)
            ...
            # last, z vs y in ax4
            hist, xe, ye = np.histogram2d(
                zp, yp, bins=nbins, weights=other_weights)
            hist = convert_to_per_pc2(hist, xe[1]-xe[0])
            Y_eo, X_eo = np.meshgrid(xe, ye)
        ...
        print("!! don't know method for plotting particles as {}; not plotting nearby particles".format(
            other_method))
    ...
    if kind == '3panel':
        ...
        fig.subplots_adjust(left=0.05, bottom=0.05, top=0.95,
                            right=0.95, hspace=0.025, wspace=0.025)

        ax1.plot([x1, x2], [y, y], lw=3, ls='-',
                 color=scalecolor, label='_nolegend_')
        ax1.text((x1+x2)/2., y+width/50, scalelabel, color=scalecolor,
                 ha='center', va='bottom', fontsize=0.75*fontsize)

        ax3.plot([x1, x2], [y, y], lw=3, ls='-',
                 color=scalecolor, label='_nolegend_')
        ax3.text((x1+x2)/2., y+width/50, scalelabel, color=scalecolor,
                 ha='center', va='bottom', fontsize=0.75*fontsize)

        ax4.plot([x1, x2], [y, y], lw=3, ls='-',
                 color=scalecolor, label='_nolegend_')
        ax4.text((x1+x2)/2., y+width/50, scalelabel, color=scalecolor,
                 ha='center', va='bottom', fontsize=0.75*fontsize)
        ...
        if label is not None:
            ax2.text(0.5, 0.1, label, ha='center', va='bottom',
                     transform=ax2.transAxes, color='k', fontsize=fontsize)

        if cbar:
            cax, kw = mpl.colorbar.make_axes(
                ax2, location='bottom', fraction=0.1, shrink=0.95, aspect=8)
            cbar = mpl.colorbar.ColorbarBase(cax, cmap=cmap, norm=cNorm, **kw)
            cbar.set_label(
                r'projected density [$M_\odot/$pc$^2$]', labelpad=15, fontsize=0.75*fontsize)
            cbar.ax.xaxis.set_ticks_position('top')
            cbar.ax.xaxis.set_label_position('top')
            cbar.ax.tick_params(axis='x', which='major',
                                pad=1, labelsize=0.5*fontsize)
        ...
        # first, x vs z in ax1
        circ = plt.Circle((0, 0), radius_to_plot,
                          facecolor='None', edgecolor=circcolor, lw=0.75)
        ax1.add_artist(circ)
        # second, x vs y in ax3
        circ = plt.Circle((0, 0), radius_to_plot,
                          facecolor='None', edgecolor=circcolor, lw=0.75)
        ...
        ax.text((x1+x2)/2., y+width/50, scalelabel,
...
def viz_part_2d(part, center_position, outbase=None, outext='png',
    ...
    try:
        if nearby_method.startswith('hist'):
            nearby_method = 'hist'
        elif nearby_method.startswith('scatter'):
            nearby_method = 'scatter'
    ...
    if rotate and rot_vec is None:
        if rotate_spec not in part:
            raise KeyError(
                "Asked to rotate on {}, but that's not in the particle dictionary".format(rotate_spec))
        if rotate_spec not in species:
            print("Adding {} to species to viz because you're asking to rotate on it, so deal with it.".format(
                rotate_spec))
    ...
    for species_name in species:
        if rotate:
            if rot_vec is None:
                ...
                RV, EV, AR = ut.particle.get_principal_axes(
                    part, species=rotate_spec, center_position=center_position, distance_max=rotate_cut)
            else:
                RV = rot_vec
            rot_pos = ut.particle.get_distances_wrt_center(
                part, species=species_name, center_position=center_position, distance_kind='vector', rotation=RV)
            particles_in_box = boxmsk(
                rot_pos, [0, 0, 0], bsize)  # already shifted
        else:
            rot_pos = part[species_name]['position'] - center_position
            particles_in_box = boxmsk(
                part[species_name]['position'], center_position, bsize)
        ...
        if verbose:
            print("Plotting {} particles in a cube of half side length = {} kpc".format(
                nbox, bsize))
            if pind is not None:
                print("{} of those ({:.1f}%) are associated with the halo"
                      .format(nmine, nmine*100./nbox))
        ...
        if figsize is None:
            figsize = (max(mpl.rcParams['figure.figsize']), max(
                mpl.rcParams['figure.figsize']))

        fig = plt.figure(figsize=figsize)
        if kind == '3panel':
            ax1 = fig.add_subplot(221, facecolor=bgcolor,
                                  aspect='equal')  # x vs z
            ax2 = fig.add_subplot(222, facecolor='w')  # nothing; maybe a label
            # x vs y -- shares the xaxis with the plot above it
            ax3 = fig.add_subplot(223, facecolor=bgcolor,
        ...
        if xp.size:
            if kind == '3panel':
                ...
                Y_eo, X_eo = np.meshgrid(xe, ye)
                im = ax1.pcolormesh(
                    X_eo, Y_eo, hist, norm=cNorm, **this_kwargs)
            ...
            if xp.size:
                if nearby_method == 'hist':
                    if kind == '3panel':
                        ...
                        Y_eo, X_eo = np.meshgrid(xe, ye)
                        im = ax1.pcolormesh(
                            X_eo, Y_eo, hist, norm=cNorm, **nearby_kwargs)
        ...
        if kind == '3panel':
            ...
            if circradius > 0:
                # first, x vs z in ax1
                circ = plt.Circle(
                    (0, 0), circradius, facecolor='None', edgecolor=circcolor, lw=0.75)
                ax1.add_artist(circ)
                # second, x vs y in ax3
                circ = plt.Circle(
                    (0, 0), circradius, facecolor='None', edgecolor=circcolor, lw=0.75)
...
def viz_with_halos(part, hal=None, species_name='star', kind='3panel',
    ...
    if rotate:
        ...
        rot_pos = ut.particle.get_distances_wrt_center(
            part, species=species_name, center_position=center_position, rotation=RV)
...
#     tickstr = ['$'+str(kk)+'$' for kk in tickloc]
    ...
    if kind == '3panel':
        ...
        hist = convert_to_per_pc2(hist, xe[1]-xe[0])
        Y_eo, X_eo = np.meshgrid(xe, ye)
        im = ax1.pcolormesh(X_eo, Y_eo, hist, norm=LogNorm(),
                            cmap=cmap, vmax=cvmax, vmin=cvmin)

        # second, x vs y in ax3
        hist, xe, ye = np.histogram2d(xp, yp, bins=nbins, weights=weights)
        hist = convert_to_per_pc2(hist, xe[1]-xe[0])
        Y_eo, X_eo = np.meshgrid(xe, ye)
        im = ax3.pcolormesh(X_eo, Y_eo, hist, norm=LogNorm(),
                            cmap=cmap, vmax=cvmax, vmin=cvmin)
        ...
        hist = convert_to_per_pc2(hist, xe[1]-xe[0])
        Y_eo, X_eo = np.meshgrid(xe, ye)
        im = ax4.pcolormesh(X_eo, Y_eo, hist, norm=LogNorm(),
                            cmap=cmap, vmax=cvmax, vmin=cvmin)
    else:
        if kind == 'edgeon':
            # want x vs z:
        ...
        hist = convert_to_per_pc2(hist, xe[1]-xe[0])
        Y_eo, X_eo = np.meshgrid(xe, ye)
        im = ax.pcolormesh(X_eo, Y_eo, hist, norm=LogNorm(),
                           cmap=cmap, vmax=cvmax, vmin=cvmin)
        if cbar:
            cbar = plt.colorbar(im, cax=cax)
            cbar.set_label(
    ...
    if kind == '3panel':
        ...
        fig.subplots_adjust(left=0.05, bottom=0.05, top=0.95,
                            right=0.95, hspace=0.075, wspace=0.075)

        ax1.plot([x1, x2], [y, y], lw=3, ls='-', color='w', label='_nolegend_')
        ax1.text((x1+x2)/2., y+width/50, scalelabel,
                 color='w', ha='center', va='bottom')

        ax3.plot([x1, x2], [y, y], lw=3, ls='-', color='w', label='_nolegend_')
        ax3.text((x1+x2)/2., y+width/50, scalelabel,
                 color='w', ha='center', va='bottom')

        ax4.plot([x1, x2], [y, y], lw=3, ls='-', color='w', label='_nolegend_')
        ax4.text((x1+x2)/2., y+width/50, scalelabel,
                 color='w', ha='center', va='bottom')

        if label is not None:
            ax2.text(0.5, 0.1, label, ha='center', va='bottom',
                     transform=ax2.transAxes, color='k', fontsize='small')
    ...
    if hal is not None:
        if rotate:
            mycen = ut.coordinate.get_coordinates_rotated(
                hal['position']-center_position, rotation_vectors=RV)
        else:
        ...
        if not len(hal_indices):
            ...
            if verbose:
                print("{} total halos in the box".format(
                    np.count_nonzero(halmsk)))
            ...
            if verbose:
                print("{} halos in the box passed the cut".format(
                    np.count_nonzero(my_msk)))
        else:
            my_msk = np.empty(hal['position'].shape[0], dtype=bool)*False
            my_msk[hal_indices] = True
        # either not doing onebyone at all, or want to do onebyone but highlighting the selected one (in which case I first plot all of the circles)
        if plot_others and ((len(onebyone_highlight)) or (not len(onebyone))):
            print("Circling {} halos in the box".format(
                np.count_nonzero(my_msk)))
            for ii in range(my_msk.shape[0]):
                if my_msk[ii]:
                    ...
                    if kind == '3panel':
                        # first, x vs z in ax1
                        circ1 = plt.Circle(
                            (cen[0], cen[2]), r, facecolor='None', edgecolor=circcolor, lw=0.75)
                        ax1.add_artist(circ1)
                        # second, x vs y in ax3
                        circ3 = plt.Circle(
                            (cen[0], cen[1]), r, facecolor='None', edgecolor=circcolor, lw=0.75)
                        ax3.add_artist(circ3)
                        # last, z vs y in ax4
                        circ4 = plt.Circle(
                            (cen[2], cen[1]), r, facecolor='None', edgecolor=circcolor, lw=0.75)
                        ...
                        if kind == 'edgeon':
                            ...
                            yi = cen[1]
                        circ = plt.Circle(
                            (xi, yi), r, facecolor='None', edgecolor=circcolor, lw=0.75)
                        ax.add_artist(circ)
        ...
        if highlight_hali is not None:
            ...
            if kind == '3panel':
                # first, x vs z in ax1
                circ1 = plt.Circle(
                    (cen[0], cen[2]), r, facecolor='None', edgecolor=onebyone_highlight, lw=0.75)
                ax1.add_artist(circ1)
                # second, x vs y in ax3
                circ3 = plt.Circle(
                    (cen[0], cen[1]), r, facecolor='None', edgecolor=onebyone_highlight, lw=0.75)
                ax3.add_artist(circ3)
                # last, z vs y in ax4
                circ4 = plt.Circle(
                    (cen[2], cen[1]), r, facecolor='None', edgecolor=onebyone_highlight, lw=0.75)
                ax4.add_artist(circ4)
            else:
                if kind == 'edgeon':
                    # want x vs z:
                    ...
                    xi = cen[0]
                    yi = cen[1]
                circ = plt.Circle((xi, yi), r, facecolor='None',
                                  edgecolor=circcolor, lw=0.75)
                ax.add_artist(circ)
            ...
            for ii in range(my_msk.shape[0]):
                if my_msk[ii]:
                    ...
                    if kind == '3panel':
                        # first, x vs z in ax1
                        circ1 = plt.Circle(
                            (cen[0], cen[2]), r, facecolor='None', edgecolor=color, lw=0.75)
                        ax1.add_artist(circ1)
                        # second, x vs y in ax3
                        circ3 = plt.Circle(
                            (cen[0], cen[1]), r, facecolor='None', edgecolor=color, lw=0.75)
                        ax3.add_artist(circ3)
                        # last, z vs y in ax4
                        circ4 = plt.Circle(
                            (cen[2], cen[1]), r, facecolor='None', edgecolor=color, lw=0.75)
                        ax4.add_artist(circ4)
                        ...
                        if kind == 'edgeon':
                            ...
                            yi = cen[1]
                        circ = plt.Circle(
                            (xi, yi), r, facecolor='None', edgecolor=color, lw=0.75)
                        ax.add_artist(circ)
                        artists = [circ]

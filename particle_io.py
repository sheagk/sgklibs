...
def read_part_wetzel(fname, types, assign_host_coordinates=False, subselect=False, quiet=False,**kwargs):
    ...
    if path.isfile(fname):
        myRead = gizmo.io.ReadClass(snapshot_name_base=snapshot_name.rsplit('_',1)[0], quiet=quiet)
...
def readsnap_wrapper(snapfile,dmo,dogasT=False,verbose=False,docosmoconvert=1):
    ...
    if dmo:
        ...
        if dogasT:  gastemp = None
    else:
        gas = readsnap(snapdir,snapnum,0,snapshot_name=snapshot_name,h0=1,cosmological=docosmoconvert)
        print("Read gas")
        star = readsnap(snapdir,snapnum,4,snapshot_name=snapshot_name,h0=1,cosmological=docosmoconvert)
        print("Read star")
    ...
    dark = readsnap(snapdir,snapnum,1,snapshot_name=snapshot_name,h0=1,cosmological=docosmoconvert)
    print("Read DM")
    dpos = dark['p'][:]
    dvel = dark['v'][:]
    dmass =dark['m'][:]*1e10

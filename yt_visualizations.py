...
from .low_level_utils import read_part_wetzel, backup_file, isint, isfloat, boxmsk
...
class ProjectParticlesYT():
    ...
    **kwargs):
    ...
    data[(ptype,'particle_position_x')] = positions[:,indices[0]]*inversions[0]
    data[(ptype,'particle_position_y')] = positions[:,indices[1]]*inversions[1]
    data[(ptype,'particle_position_z')] = positions[:,indices[2]]*inversions[2]
    ...
    ds = yt.load_particles(data, length_unit=kiloparsec, mass_unit=Msun, bbox=bbox, n_ref=nref, periodicity=(False, False, False))
    
    method = kwargs.pop('method', 'mip')

    field_name = ptype+'_density'
    prj = yt.ProjectionPlot(ds, axis, ('deposit',field_name), method=method, center=center, width=(bsize,'kpc'), **kwargs)
    ...
    if (vmin is not None) and (vmax is not None):
        ...
        prj = prj.set_zlim(field_name, zmin=vmin, zmax=None, dynamic_range=dynamic_range)
    elif vmax is not None and dynamic_range is not None:
        prj = prj.set_zlim(field_name, zmin=None, zmax=vmax, dynamic_range=dynamic_range)
    ...
    if noaxes:
        ...
        if scalebar is None:
            ...
            prj = prj.annotate_scale(min_frac=minfrac,max_frac=maxfrac,pos=pos,text_args=text_args)
        elif isinstance(scalebar, float) or isinstance(scalebar, int):
            if scalebar > 0:
                yloc = 0.05
                xloc = ((scalebar/bsize)/2.0 + 0.075)
                pos = (xloc, yloc)
                prj = prj.annotate_scale(pos=pos,coeff=scalebar,unit='kpc',text_args=text_args)
            else:
                print("Not adding a scalebar to an image with a total width of {} kpc".format(bsize))
        else:
            print("Not adding a scalebar to an image with a total width of {} kpc".format(bsize))

    if halos_to_add is not None:
        prj = self.add_halos(prj, halos_to_add, img_rotate=img_rotate, axis=axis, center=orig_center, **haloargs)

    if circle_radius is not None:
        prj = prj.annotate_sphere(center=[0,0,0], radius=(circle_radius, 'kpc'), circle_args=circle_args)
    ...
    if text_to_annotate is not None:
        if 'text_args' not in text_annotate_kwargs:
            ...
            text_annotate_kwargs['text_args']['fontsize'] = fontsize
        prj = prj.annotate_text(text_loc, text_to_annotate, **text_annotate_kwargs)
    ...
    def add_halos(self, prj, hal, 
        ...
        radiusprop='radius', 
        circle_args={'facecolor':None, 'edgecolor':'white', 'linewidth':1.5},
        img_rotate=0, axis=0,
        ):
        '''
        ...
        for idx in hal_indices:
            c = np.array([hal_positions[idx][indices[ii]]*inversions[ii] for ii in range(3)])
            prj.annotate_sphere(c, radius=(hal[radiusprop][idx], 'kpc'), circle_args=circle_args)
        return prj
